...
const kpiService = {
  ...
  async obtenerKpis(filtros = {}) {
    try {
      ...
      if (filtros.direccion) query = query.eq('direccion', filtros.direccion);
      if (filtros.limit) query = query.limit(filtros.limit);
      if (filtros.offset) query = query.range(filtros.offset, filtros.offset + (filtros.limit || 100) - 1);
  ...
  async obtenerKpisPorArea(area, limit = 50) {
    ...
    if (error) throw new Error(`Error al obtener KPIs del área: ${error.message}`);
    return data || [];
  },
  ...
  async obtenerUltimosKpisPorArea(area) {
    ...
    if (errorIndicadores) throw new Error(`Error al obtener indicadores: ${errorIndicadores.message}`);
  ...
  async obtenerHistoricoKpi(area, indicador, dias = 30) {
    ...
    if (error) throw new Error(`Error al obtener histórico del KPI: ${error.message}`);
    return data || [];
  },
  ...
  async obtenerResumenKpis(area = null) {
    ...
    const { data, error } = await query;
    if (error) throw new Error(`Error al obtener resumen de KPIs: ${error.message}`);
    return data || [];
  },
  ...
  async obtenerEstadisticasArea(area, dias = 30) {
    ...
    if (!data || data.length === 0) {
      return { total_registros: 0, indicadores_unicos: 0, porcentaje_cumplimiento: 0, ultimos_registros: [] };
    }

    const indicadoresUnicos = [...new Set(data.map(k => k.indicador))];
    const registrosConMeta = data.filter(k => k.cumple_meta !== null && k.cumple_meta !== undefined);
    const cumplimiento = registrosConMeta.filter(k => k.cumple_meta === true).length;
    const porcentajeCumplimiento = registrosConMeta.length > 0 ? (cumplimiento / registrosConMeta.length) * 100 : 0;
    ...
    const porIndicador = indicadoresUnicos.map(indicador => {
      ...
      const conMeta = registros.filter(r => r.cumple_meta !== null && r.cumple_meta !== undefined);
      const cumplen = conMeta.filter(r => r.cumple_meta === true).length;
      const porcentajeCumple = conMeta.length > 0 ? (cumplen / conMeta.length) * 100 : null;

      return {
        indicador,
        total_registros: registros.length,
        valor_promedio: promedio.toFixed(2),
        porcentaje_cumplimiento: porcentajeCumple !== null ? porcentajeCumple.toFixed(2) : 'N/A',
  ...
  async obtenerTendenciaKpi(area, indicador, cantidadRegistros = 10) {
    ...
    if (error) throw new Error(`Error al obtener tendencia: ${error.message}`);
    if (!data || data.length === 0) return { tendencia: 'sin_datos', registros: [] };

    const valores = data.map(d => parseFloat(d.valor)).reverse();
    if (valores.length < 2) return { tendencia: 'insuficientes_datos', registros: data.reverse() };

    let ascendentes = 0, descendentes = 0;
    ...
    let tendencia;
    if (ascendentes > descendentes * 1.5) tendencia = 'ascendente';
    else if (descendentes > ascendentes * 1.5) tendencia = 'descendente';
  ...
  async aggregateKpisForArea(area, periodo) {
    ...
    if (errMes) throw new Error(`Error al leer registros del mes: ${errMes.message || JSON.stringify(errMes)}`);
    ...
    if (errYtd) throw new Error(`Error al leer registros YTD: ${errYtd.message || JSON.stringify(errYtd)}`);
    ...
    const kpis = indicadores.map(ind => {
      ...
      return {
        ...
        metaTipo: ultimoMes && ultimoMes.meta !== null && ultimoMes.meta < 0 ? 'menorIgual' : 'mayorIgual',
    ...
    if (sinDato.length) analisis.push(`Indicadores sin registro este mes: ${sinDato.slice(0, 10).join(', ')}${sinDato.length > 10 ? ' (y más...)' : ''}.`);
    ...
    if (peor.length) analisis.push('Principales desviaciones respecto a meta: ' + peor.map(p => `${p.label} (${p.valor_mes}${p.unidad || ''} vs meta ${p.meta})`).join('; ') + '.');

    // Proyectos y plan_accion si existen
    let proyectos = [];
    try {
